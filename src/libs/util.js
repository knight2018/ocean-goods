...
export const TOKEN_KEY = 'oceanShopping'
...
export const getMenuByRouter = (list, access) => {
  ...
  if (localStorage.menuPermission !== '01') {
    ...
    if (menuPermission.length) {
      menuPermission.forEach(item => {
        if (item.checkAllGroup.length) {
          arrList.push(item.name)
          item.checkAllGroup.forEach(items => {
            arrList.push(items)
          })
        }
      })
...
export const setTagNavListInLocalstorage = list => {
  localStorage.tagNaveListOceanShopping = JSON.stringify(list)
}
/**
 * @returns {Array} 其中的每个元素只包含路由原信息中的name, path, meta三项
 */
export const getTagNavListFromLocalstorage = () => {
  const list = localStorage.tagNaveListOceanShopping
